...
exports.add = async (req, res) => {
    ...
    try
    {
        ...
        if(error.code === 11000){
            res.status(400).json({message:`Ya existe un producto con ese nombre: ${req.body.name}`});
        }
        else {
...
exports.update = async(req, res, next) => {
    try{
        ...
        if(error.code === 11000){
            res.status(400).json({message:`Ya existe un producto con el sku: ${req.body.name}`});
        }
        else {
            res.status(400).json({message: 'Error'});
        }
